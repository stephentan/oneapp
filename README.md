...
- creating users, and logging in and out [~/modules/core-module/models/user.server.ts](~/modules/core-module/models/user.server.ts)
...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create shoprabbit-4ca1
  fly apps create shoprabbit-4ca1-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app shoprabbit-4ca1
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app shoprabbit-4ca1-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'shoprabbit-4ca1-staging' does not match app name in config file 'shoprabbit-4ca1'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name shoprabbit-4ca1-db
  fly postgres attach --app shoprabbit-4ca1 shoprabbit-4ca1-db

  fly postgres create --name shoprabbit-4ca1-staging-db
  fly postgres attach --app shoprabbit-4ca1-staging shoprabbit-4ca1-staging-db
  ```
